...
$Config['title']= str_replace('${title}', __('Popular'), get($Settings, "data.anime_title", "seo"));
$Config['description']  = str_replace('${title}', __('Popular'), get($Settings, "data.anime_description", "seo"));
$SearchPage = '?sorting=popular&';
}elseif($Filter['sorting'] == 'newest') {
$OrderBy = 'ORDER BY posts.id DESC';
$Config['header']   = __('Newest Anime');
$Config['title']= str_replace('${title}', __('Newest'), get($Settings, "data.anime_title", "seo"));
$Config['description']  = str_replace('${title}', __('Newest'), get($Settings, "data.anime_description", "seo"));
$SearchPage = '?sorting=newest&';
}elseif($Filter['sorting'] == 'released') {
$OrderBy = 'ORDER BY posts.create_year DESC';
$Config['header']   = __('Popular Anime');
$Config['title']= str_replace('${title}', __('Featured'), get($Settings, "data.anime_title", "seo"));
$Config['description']  = str_replace('${title}', __('Featured'), get($Settings, "data.anime_description", "seo"));
...
$Where .= 'WHERE posts.type = "serie" AND posts.anime = "1" AND posts.status = "1" AND ';
if($Filter['quality']) {
